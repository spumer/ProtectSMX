...
std::vector<char> KEY({
	0x5F,0x3B,0x35,0x2E,0x5D,0x39,0x34,0x2D,0x33,
	0x31,0x3D,0x3D,0x2D,0x25,0x78,0x54,0x21,0x5E,
	0x5B,0x24,0x00
});
...
class Blowfish {
public:
  explicit Blowfish(const std::vector<char> &key);
...
FILE* protect_read_smx(const char* file, const char* mode)
{
	FILE *fp = fopen(file, "rb");
	if (!fp) return fp;

	FILE *plugin_fp;
	Blowfish bfengine(KEY);

	FileType ftype = DetectFileType(fp);
	FileReader reader(fp);
	fclose(fp);

	const char *buffer = reinterpret_cast<const char *>(reader.buffer());
	std::vector<char> content(buffer, buffer + reader.length());

	if (ftype == FileType::UNKNOWN)
	{
		// Скорее всего файл был ранее нами зашифрован
		// Расшифровываем и возвращаем плагин

		auto&& encrypted = content;

		auto decrypted = bfengine.Decrypt(encrypted);

		auto decrypted_size = decrypted.size();
		char* result = (char*)malloc(decrypted_size);
		std::copy(decrypted.begin(), decrypted.end(), result);

		plugin_fp = fmemopen(result, decrypted_size, mode);
	}
	else
	{
		// Плагин не зашифрован, шифруем его на диске
		// и возвращаем незашифрованную копию

		auto&& decrypted = content;

		auto encrypted = bfengine.Encrypt(decrypted);
		fp = fopen(file, "wb");
		fwrite(encrypted.data(), sizeof(char), encrypted.size(), fp);
		fclose(fp);
